...
case 'totag':
            if (!Fg.key.fromMe && !isOwner && !isGroupAdmins) return reply(mess.admin)
            if ((isMedia && !Fg.message.videoMessage || isQuotedSticker) && args.length == 0) {
            encmedia = isQuotedSticker ? JSON.parse(JSON.stringify(Fg).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : dev
            file = await Fg.downloadAndSaveMediaMessage(encmedia, filename = getRandom())
            value = args.join(" ")
            ...
            fs.unlinkSync(file)
            } else if ((isMedia && !Fg.message.videoMessage || isQuotedImage) && args.length == 0) {
            encmedia = isQuotedImage ? JSON.parse(JSON.stringify(Fg).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : dev
            file = await Fg.downloadAndSaveMediaMessage(encmedia, filename = getRandom())
            value = args.join(" ")
            var group = await Fg.groupMetadata(from)
            var member = group['participants']
            ...
            fs.unlinkSync(file)
        } else if ((isMedia && Fg.message.videoMessage || isQuotedAudio) && args.length == 0) {
            encmedia = isQuotedAudio ? JSON.parse(JSON.stringify(Fg).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : dev
            file = await Fg.downloadAndSaveMediaMessage(encmedia, filename = getRandom())
            value = args.join(" ")
            var group = await Fg.groupMetadata(from)
            var member = group['participants']
            ...
            fs.unlinkSync(file)
        }  else if ((isMedia && !Fg.message.videoMessage || isQuotedVideo) && args.length == 0) {
            encmedia = isQuotedVideo ? JSON.parse(JSON.stringify(Fg).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : dev
            file = await Fg.downloadAndSaveMediaMessage(encmedia, filename = getRandom())
            value = args.join(" ")
            var group = await Fg.groupMetadata(from)
            var member = group['participants']
